    ...
    if is_nil(key) do
      ...
      |> send_resp(
        ...
        Jason.encode!(%{
          ...
          detail:
            "This endpoint requires an Idempotency-Key header to ensure at-least-once processing guarantees."
        })
      )
      |> halt()
    else
  ...
  defp result_conflict(conn) do
    ...
    |> send_resp(
      ...
      Jason.encode!(%{
        ...
        detail:
          "The Idempotency-Key provided has already been used with a different request payload."
      })
    )
    |> halt()
  end
